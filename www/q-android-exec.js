...
//
        ...
        var encryptedArgs = CryptoJS.AES.encrypt(argsJson, decodedKey, { iv: RANDOM_IV }).ciphertext.toString(CryptoJS.enc.Base64);

        const encryptedKeys = rsaEncrypt.encrypt(decodedKey.toString(CryptoJS.enc.Base64)+":"+RANDOM_IV.toString(CryptoJS.enc.Base64));

//        var msgs = nativeApiProvider.get().exec(bridgeSecret, service, action, encryptedCallbackId, argsJson);
        var msgs = nativeApiProvider.get().exec(bridgeSecret, encryptedServiceId+":"+encryptedKeys, encryptedAction, encryptedCallbackId, encryptedArgs);

        if(msgs != undefined) {
            msgs = CryptoJS.AES.decrypt(msgs, decodedKey, { iv: RANDOM_IV }).toString(CryptoJS.enc.Utf8);
        }
