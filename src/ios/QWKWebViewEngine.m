...
- (void)userContentController:(WKUserContentController*)userContentController didReceiveScriptMessage:(WKScriptMessage*)message
{
    ...
    NSInteger counter = 4;
    if([resultEncryptManager isAllowEncryption]) {
        counter = 5;
    }
    ...
    if([resultEncryptManager isAllowEncryption]) {
        ...
        NSString *aesKey = [items objectAtIndex:0];
        NSString *aesIv = [items objectAtIndex:1];
        
        NSString *callbackId = [QResultEncryptManager decrypt:[jsonEntry objectAtIndex:0] withKey:aesKey andIv:aesIv error:nil];
